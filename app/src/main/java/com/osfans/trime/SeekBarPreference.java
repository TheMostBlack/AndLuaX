...
public class SeekBarPreference extends Preference implements OnSeekBarChangeListener {
    ...
    private CharSequence getValue(int progress) {
        if (getKey().endsWith("_gap"))
            return Float.toString(((float) progress) / 20);
        if (getKey().endsWith("border") || getKey().endsWith("_gap"))
            return Float.toString(((float) progress) / 10);
        if (getKey().endsWith("_size") || getKey().endsWith("_corner"))
            return Float.toString(((float) progress));
        ...
        switch (getKey()) {
            ...
            case "repeat_interval":
                ...
                break;
            case "key_alpha":
                progress = progress * 100 / 255;
                break;
            case "keyboard_width":
                progress = progress * 10 / 20 + 50;
                break;
