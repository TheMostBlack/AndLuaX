...
public class PhraseKeyboard extends Keyboard {
    ...
    public PhraseKeyboard(Context context, int height) {
        super(context);
        this.height = height;
    ...
    private Map<String, Object> getKeysMap() {
        ...
        if (trime != null) {
            ...
            for (int i = 0; i < pageSize; i++) {
                ...
                if (i < len) {
                    String s = cb.get(i + pageSize * currPage);
                    if (s.length() > 6) {
        ...
        map.put("name", String.format("常用短语,第%d页", currPage + 1));
        map.put("lock", false);
        map.put("keys", keys);
