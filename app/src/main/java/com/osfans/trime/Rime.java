 ...
 */
@SuppressWarnings({"JniMissingFunction", "JavaJniMissingFunction"})
public class Rime {
    /**
     * Rime編碼區
     */
    ...
    public static class RimeContext {
        ...
        public RimeCandidate[] getCandidates() {
            return size() == 0 ? new RimeCandidate[0] : menu.candidates;
        }
    }
    ...
    public static class RimeSchema {
        ...
        public void check() {
            ...
            Set<String> set = Config.get().getFilterSwitches();
            for (Iterator it = switches.iterator(); it.hasNext(); ) {
                Map<String, Object> o = (Map<String, Object>) it.next();
                if (!o.containsKey("states") || set.contains(o.get("name")))
                    it.remove();
            }
            List<Map<String, Object>> list = Config.get().getCustomCandidate();
    ...
    private static void initSchema() {
        Function.printStackTrace("initSchema");
    ...
    public static RimeCandidate[] getCandidates() {
        if(isComposing()&&mContext.size()==0&&mRimeCandidate==null){
            ...
            //rc.comment=getCompositionText();
            mRimeCandidate=new RimeCandidate[]{
                  rc
            } ;
        }
    ...
    public static int selectSchemaId(String schema_id) {
        if(mSchemaList==null)
            return -1;
        for (Object o : mSchemaList) {
            Map<String, String> m = (Map<String, String>) o;
            if (m.get("schema_id").contentEquals(schema_id)) {
    ...
    public static Rime get(boolean full_check) {
        ...
        if (self == null) {
            try {
                throw new RuntimeException("full_check " + full_check);
            } catch (Exception e) {
                e.printStackTrace();
            }
            if (full_check) Config.deployOpencc();
            self = new Rime(full_check);
            android.util.Log.w("rime", "full_check: ");
        }
        return self;
    }
